...
/*	*	*	 SETUP	*	*	 */
	...
	// NAV
	$nav = array('INICIO', 'PERFIL PROFESIONAL', 'RAMAS DE DERECHO', 'COLABORADORES', 'CONTACTO');
	$navigation="";
	...
	if(count($nav) > 0){
		$navigation.='<ul>';
		foreach($nav as $n){
			$navigation.= '<li id="_'.cname($n).'">'.cLinks($n).'</li>';
		}
		$navigation.='</ul>';
	}
	# set active section
	...
	function cLinks($name, $class=''){
		...
		if($inner){
			return '<a href="'.cname($inner."/".$name).'.html" class="'.active(cname($name)).$cl.'">'.$name.'</a>';
		} else {
			return '<a href="'.cname($name).'.html" class="'.active(cname($name)).$cl.'">'.$name.'</a>';
		}
	}
